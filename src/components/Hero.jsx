...
const Hero = () => {
  const fullHeadline = "Rasel Mahmud — I can turn data into decisions with clear dashboards and user-friendly web experiences."
  const roles = ['Data Analysis', 'Frontend Development', 'UX Research']
  ...
  // Render headline with "Rasel" in blue
  const renderHeadline = () => {
    const raselIndex = headlineText.indexOf('Rasel')
    if (raselIndex === -1) {
      return headlineText
    }
    return (
      <>
        <span className="text-blue-600">{headlineText.slice(0, 5)}</span>
        <span>{headlineText.slice(5)}</span>
      </>
    )
  }
        ...
        <div className="md:w-1/2 text-center md:text-left mb-8 md:mb-0 order-2 md:order-1">
          <div className="glass rounded-3xl p-8 md:p-10 mb-6 backdrop-blur-md border border-white/20 shadow-lg">
            ...
            <div className="flex flex-col sm:flex-row justify-center md:justify-start space-y-4 sm:space-y-0 sm:space-x-4 mb-8">
              ...
              >
                <span className="group-hover:tracking-wide transition-all duration-300">View Projects</span>
              ...
              <a 
                ...
                className="group glass text-gray-900 px-10 py-4 rounded-xl text-lg font-bold hover:bg-white/70 transition-all duration-300 transform hover:scale-105 shadow-xl hover:shadow-2xl border-2 border-blue-200 hover:border-blue-400 flex items-center justify-center gap-3"
              >
            ...
            <div className="flex flex-wrap gap-3 mb-8 justify-center md:justify-start">
              <a 
                ...
                target="_blank" 
                rel="noopener noreferrer"
                className="group flex items-center gap-2 glass px-5 py-3 rounded-xl hover:bg-white/50 transition-all transform hover:scale-105 shadow-md border border-white/40"
              >
              ...
              <a 
                href="https://github.com/RaselMahmud7" 
                target="_blank" 
                rel="noopener noreferrer"
                className="group flex items-center gap-2 glass px-5 py-3 rounded-xl hover:bg-white/50 transition-all transform hover:scale-105 shadow-md border border-white/40"
              >
                <svg className="w-5 h-5 text-gray-800 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v 3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z"/></svg>
              ...
              <a 
                ...
                className="group flex items-center gap-2 glass px-5 py-3 rounded-xl hover:bg-white/50 transition-all transform hover:scale-105 shadow-md border border-white/40"
              >
                <svg className="w-5 h-5 text-red-600 group-hover:scale-110 transition-transform" fill="currentColor" viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-1.99.9-1.99 2L2 18c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2zm0 4l-8 5-8-5V6l8 5 8-5v2z"/></svg>
