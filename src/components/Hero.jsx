...
const Hero = () => {
  const fullHeadline = "Rasel Mahmud — I can turn data into decisions."
  ...
  // Render headline with colored highlights
  const renderHeadline = () => {
    const text = headlineText
    const raselMahmud = "Rasel Mahmud"
    const dataToDecisions = "data into decisions"
    
    // Find positions
    const raselEnd = text.indexOf(raselMahmud) + raselMahmud.length
    const dataStart = text.indexOf(dataToDecisions)
    const dataEnd = dataStart + dataToDecisions.length
    
    if (text.length < raselMahmud.length) {
      // Still typing name
      return <span className="text-blue-600">{text}</span>
    } else if (text.length < dataStart) {
      // Between name and "data"
      return (
        <>
          <span className="text-blue-600">{raselMahmud}</span>
          <span>{text.slice(raselEnd)}</span>
        </>
      )
    } else if (dataStart !== -1 && text.length >= dataStart && text.length <= dataEnd) {
      // Typing "data into decisions"
      return (
        <>
          <span className="text-blue-600">{raselMahmud}</span>
          <span>{text.slice(raselEnd, dataStart)}</span>
          <span className="text-blue-600 font-bold">{text.slice(dataStart)}</span>
        </>
      )
    } else if (dataStart !== -1 && text.length > dataEnd) {
      // Complete with highlights
      return (
        <>
          <span className="text-blue-600">{raselMahmud}</span>
          <span>{text.slice(raselEnd, dataStart)}</span>
          <span className="text-blue-600 font-bold">{dataToDecisions}</span>
          <span>{text.slice(dataEnd)}</span>
        </>
      )
    } else {
      return text
